...
program eos_table_maker
  ...
  initial_eos_table_name = "./original_tables/sfhx_frdm_sc_v1.03.bin"
  Hempel = .true.
  lump_into_np = .false.
  ...
  logrhomax = 16.220249d0 !global maximum of final table
        ...
        do j=1,nlogtemp
           do k=1,nye
              ...
              if(abs(dm).gt.1.0d-4 .or. abs(dz)/ye(k).gt.1.0d-4) then
                 c1 = c1 + 1
                 write(*,*) i,j,k,eos_table(i,j,k,ixn),eos_table(i,j,k,ixp), &
                   eos_table(i,j,k,ixa),eos_table(i,j,k,ixh),eos_table(i,j,k,ixt), &
                   eos_table(i,j,k,ixd),eos_table(i,j,k,ix3he), &
                   eos_table(i,j,k,ix4li)
                 write(*,*) i,j,k
