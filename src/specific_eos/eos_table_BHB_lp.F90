...
program eos_table_maker
  ...
  initial_eos_table_name = "./original_tables/bhb_lp_frdm_sc_v1.02.bin"
  Hempel = .true.
  lump_into_np = .false.
  ...
  nt_rho = 303 !number of density entries in initial table
  nvars = 22 !columns in the initial table - 3 (don't care about ye
             !and rho etc., know those)

  !size you want to final table in
  !denstiy
  nlogrho = 234 !number of density bins
  logrhomin = 2.2202492d0 !global minimum of final table
  logrhomax = 15.30024d0 !global maximum of final table
...
!.....22        lambda mass fraction X_lambda
  ...
  imue     = 23 !chemical potential of electron, MeV, includes rest
                !mass, not part of initial table but part of final

  !extra Hempel variables
  ixd = 18
  ixt = 19
  ix3he = 20
  ix4li = 21
  ...
  noelectrons = .false. !experimental, should be false for anything useful
        ...
        do j=1,nlogtemp
           do k=1,nye
              ...
              dm = eos_table(i,j,k,ixn) + eos_table(i,j,k,ixp) + &
                   ...
                   eos_table(i,j,k,ix4li) + eos_table(i,j,k,ixL) - 1.0d0
           ...
           do j=1,nlogtemp
              do k=1,nye
                 ...
                 eos_table(i,j,k,ixn) = eos_table(i,j,k,ixn) + 0.5d0*eos_table(i,j,k,ixd)+2.0d0*eos_table(i,j,k,ixt)/3.0d0+ &
                      eos_table(i,j,k,ix3he)/3.0d0+eos_table(i,j,k,ix4li)/4.0d0 + eos_table(i,j,k,ixL)
              enddo
           enddo
        enddo
  ...
  do k=1,nye
     do j=1,nlogtemp
        do i=1,nlogrho
           ...
           if (eos_table(i,j,k,ipress).lt.0.0d0) then
              write(*,*) ye(k),logtemp(j),logrho(i)
              write(*,*) eos_table(i,j,k,ipress)
           endif
