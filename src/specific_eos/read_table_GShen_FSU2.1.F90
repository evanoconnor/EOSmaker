     ...
     do k=1,nt_yp
        ...
        do i=1,nt_rho
           ...
           read(473,*) table_logtemp(j),table_yp(k),table_logrho(i), &
                (table_data(j,k,i,l), l=1,nvarsintable)

           table_logtemp(j) = log10(table_logtemp(j))

           if (i.eq.1.and.k.eq.1) then
              write(*,*) "temp",j,10.0d0**table_logtemp(j)
           endif
           if (j.eq.1.and.k.eq.1) then
              write(*,*) "rho",i,10.0d0**table_logrho(j)
           endif
           if (i.eq.1.and.j.eq.1) then
              write(*,*) "ye",k,table_yp(k)
           endif
           ...
           my_ye = table_data(j,k,i,ixp)+0.5d0*table_data(j,k,i,ixa)+table_data(j,k,i,izbar)* &
                table_data(j,k,i,ixh)/table_data(j,k,i,iabar)
